...
with DAG(
    ...
    file_download_step = BashOperator(
        ...
        bash_command='wget https://s3.sa-east-1.amazonaws.com/ckan.saude.gov.br/SGL/2021/uf=PB/lote=1/part-00000-f1f02999-8ba2-40f4-8598-df433a6f0190.c000.csv -P ' + LOCAL_DOWNLOAD,
    )

    file_rename_step = BashOperator(
        task_id='file-rename',
        bash_command='cd ' + LOCAL_DOWNLOAD + ' && mv part-00000-0508a0cd-d93a-42d4-b285-50673d466ef5.c000.csv sindrome-gripal.csv',
    )


    @task(task_id="print-csv-file")
    def print_csv(ds=None, **kwargs):
        df = pd.read_csv(LOCAL_DOWNLOAD + 'sindrome-gripal.csv', sep=";", encoding='utf-8',
                         low_memory=False)
    ...
    def send_file(ds=None, **kwargs):
        client.fput_object("pendentes", "sindrome-gripal.csv",
                           LOCAL_DOWNLOAD + "sindrome-gripal.csv")
